...
Teaching Phase 3 demonstrates lexical path checks, a shell denylist, and JSONL trace. Phase H is **L2 for single-user trusted-host scope** as of `d22ce6e`. Current state and exclusions are tracked in the [maturity matrix](./docs/maturity.md).
...
| Edit commit | h-edit-integrity-001 L2: strict endpoints/parent containment, atomic replace, cleanup truth, preallocated staged failures, final symlink, Agent/remember, signaled-diff ownership | reviews 01–04 review/fix cycle + final Oracle/main Gate passed; no fsync/power-loss, metadata, multi-file, or hostile-FS-race claim |
...
## Shell/process boundary (`h-shell-001` done)

`run_shell` is foreground and synchronous. It passes execute permission and descriptor-selected `protect|off` policy, then invokes Zig 0.16 `std.process.run`. Policy denial returns one fixed generic Tool body without a command preview. Runtime emits stable `shell-v1` headers, uses a one-time absolute `.awake` capture deadline, and caps stdout/stderr at 30 KiB each. Valid UTF-8 remains exact; an invalid whole stream is represented as padded standard base64 directly in the checked final body. Capture overflow and base64 body expansion are distinct bounded `shell_output_limit limit_scope=capture|body_encoding` results with no partial output. Runtime diagnostics omit command text, configured/test shell path, and raw Zig error names; `OutOfMemory` remains hard typed. This remains a trusted-host runner, not workspace path containment or an OS sandbox.

Zig's post-spawn error unwind installs `defer child.kill(io)`, which synchronously kills/reaps the spawned **direct child** before timeout/capture-output-limit returns; body-encoding limit is evaluated only after normal wait/reap. Permanent macOS/Linux fixtures record that direct PID and prove only that it is absent after the handler returns; the pinned Zig source supplies the unwind mechanism. They make no end-to-end wall-clock bound. Independent re-review, Oracle, and the merged main std/curl Gate passed. This closes the synchronous shell module only; the later final integration audit failed on two separate file-tool contracts and did not reopen shell.
...
- Model-plane / openai-zig HTTP diagnostics print **status + body length only** — never Authorization, raw body text, or hex dumps. On provider failure, zag-ai `provider_diag` may persist status + parsed `error.type`/`code`/`param` + a truncated, key-shape-scrubbed `error.message` plus request-shape counts to `.zag/logs/provider-last.json` (and append `provider.jsonl`). HTTP bake-off prints fixed `BASE_URL=configured` (never raw base URL).
...
| ~~default Agent policy/containment composition~~ | **done** h-integration-001; fresh 11-sentence audit PASS, panel SHIP, merged-main Gate passed |
...
| ~~target-preserving `write_file` / `search_replace` commit faults~~ | **done P0** h-edit-integrity-001; reviews 01–04 review/fix cycle + final Oracle + merged-main default/curl Gate passed |
| ~~bounded read/list/grep/glob bodies + explicit incomplete cutoffs~~ | **done P1** h-read-search-bounds-001; reviews 01–10 review/fix cycle + final PASS/SHIP + merged-main Gate passed |
