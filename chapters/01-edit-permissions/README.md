...
本地 descriptor：`risk=write`，`workspace=path_field("path")`。H 的 h-edit-integrity-001 先验证 file endpoint 与 canonical target/parent containment，再准备完整 bytes、success body 与所有稳定 failure bodies，之后才打开 resolved parent/创建 atomic temp；写完、writer flush、Guard 重检后 replace。staging 后 cleanup/classification 只转移预备 body ownership，不再 allocation；commit 后 optional git diff 的 stdout 在 exited/non-exited term 都只有一个 owner，capture/term/format 失败保留 mandatory success。contained final file symlink 与 cleanup truth 已通过独立/main Gate；这里不表示 fsync/power-loss durability。
...
本地 descriptor：`risk=execute`，`shell=command_argument`（policy 不靠名字猜）。Policy deny 的 Tool body 固定且不回显命令。同步 handler 使用 `shell-v1` first line 区分 outcomes 与 `limit_scope=capture|body_encoding`；有效 UTF-8 原样，无效 whole stream 用 padded standard base64；每流 30 KiB 且不伪造 partial output。这里只覆盖 foreground direct child，不是 PTY/background/process-tree supervisor。
...
**h-shell-001 Gate 已通过：** fixed deny、UTF-8/base64、scoped limits、N/N+1、direct-PID/parsed single-call trace fixtures 已通过独立/Oracle/main std+curl。Final audit 的 single-file edit blocker 也由 `h-edit-integrity-001` 关闭：strict endpoint/parent containment、atomic replace、cleanup truth、preallocated staged failures、contained final symlink、yolo Agent recovery、core lexical remember 和 real signaled-diff child 均完成 reviews 01–04 review/fix cycle，并通过 final Oracle/main Gate。`h-read-search-bounds-001` 已关闭限定 read/search L2：64 KiB body、完整 `fs-v1` cutoff marker、N/N+1/growth、walker/source/binary/pattern、fixed exclusions、descriptor/Agent evidence 通过 reviews 01–10 cycle、final PASS/SHIP 和 merged-main Gate。H remember key 按 exact lexical request path；alias re-prompt 且 Guard 始终重检。**Phase H 已 closeout 为 L2（单用户、受控本机）：** `h-integration-001` 在 `d22ce6e` 通过 fresh 11-sentence audit（11/11 PASS，panel SHIP）。仍不宣称：SDK-ready、headless-ready、OS sandbox、mid-flight shell cancellation/process-tree ownership、fsync/power-loss edit durability、完整 Plan UX 与 canonical path-domain policy。
