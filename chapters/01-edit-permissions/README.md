...
| Flag | 含义 |
|------|------|
| `--ask`（默认） | 只读自动过；`write_file` / `search_replace` / `run_shell` 要确认 |
| `--yolo` | 全部自动允许 |
| `-p ask\|yolo` | 同上 |
...
整文件 `write_file` + 全局 ask/yolo 只够 **Teaching** 演示。  
**H2 已补：** `search_replace`（唯一内容锚点）、`grep` / `glob`、写后可选 diff —— 见 [tools-edit](../../docs/modules/tools-edit.md) / [gaps/01-edit](../../docs/gaps/01-edit.md)。权限矩阵仍属 H3。
