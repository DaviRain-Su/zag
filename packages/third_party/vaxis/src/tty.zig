...
pub const PosixTty = struct {
    ...
    /// Zag deviation (tui-vaxis-001): true when `fd` was opened by us
    /// (/dev/tty fallback). A borrowed stdin fd must never be closed on
    /// deinit.
    owned_fd: bool = false,
    ...
    /// initializes a Tty instance by opening the process tty and "making it
    /// raw". A signal handler is installed for SIGWINCH. No callbacks are
    /// installed, be sure to register a callback when initializing the event
    /// loop.
    ///
    /// Zag deviation (tui-vaxis-001): upstream opens `/dev/tty` (the
    /// controlling terminal). Zag's terminal contract is stdin/stdout-based,
    /// and the PTY process fixture runs the child WITHOUT a controlling
    /// terminal (macOS teardown constraint), where /dev/tty would resolve to
    /// the test runner's terminal instead of the PTY slave. When stdin is a
    /// tty we use it directly; /dev/tty remains the fallback for consumers
    /// whose stdin is not a tty.
    pub fn init(io: std.Io, buffer: []u8) !PosixTty {
        // Open the tty
        var f: std.Io.File = undefined;
        var owned_fd = false;
        const stdin_file: std.Io.File = .{
            .handle = posix.STDIN_FILENO,
            .flags = .{ .nonblocking = false },
        };
        if (std.Io.File.isTty(stdin_file, io) catch false) {
            f = stdin_file;
        } else {
            f = try std.Io.Dir.openFileAbsolute(io, "/dev/tty", .{ .mode = .read_write });
            owned_fd = true;
        }
