...
pub fn main() !void {
    ...
    if (compat.skipIfDeepSeek(conf.base_url, "transcriptions")) return;

    const response = client.audio().create_transcription_from_path(gpa, .{
        ...
        switch (err) {
            errors.Error.NotFoundError => {
                std.debug.print("transcriptions endpoint unavailable on this provider (HTTP 404).\n", .{});
                return;
            },
            else => return err,
        }
