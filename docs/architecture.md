...
> Teaching Phase 0–3 = 骨架已落地；Production Floor（Phase H）仍由 P1 synchronous shell/runtime observability 与最终 exit audit 阻塞。
...
| shell policy/runtime | `shell_policy.zig` + coding `runtime/edit_tools.zig` | L1；denylist 非 sandbox；`h-shell-001` 补 stable runtime/budget/direct-child/trace contract |
| trace | `zag-agent-core/src/trace.zig` | L2；versioned、truthful unique terminal、atomic persistence、redaction；shell projection fixture open |
...
| run_shell | ✅ permission + descriptor-selected policy + synchronous basics | `h-shell-001` shell-v1/budget/direct-child/trace Gate；mid-flight cancel/process tree/OS sandbox remain post-H |
