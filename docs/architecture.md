...
> Teaching Phase 0–3 = 骨架已落地；Production Floor（Phase H）仍有 P0/P1 correctness gate。
...
| **L2 Model plane** | models / sampler / sampling-types | resolve、catalog、WireAdapter、stream、errors | L1+；OpenAI-compatible + Anthropic 已有，deadline/cancel 待 H6 |
...
| edit/search | `zag-coding-agent/src/runtime/*` | L1+；search_replace/grep/glob 已落地 |
| provider | core Provider + zag-ai WireAdapter | L1+；OpenAI-compatible + Anthropic 已落地，deadline/cancel 未闭合 |
...
| WireAdapter（OpenAI-compatible + Anthropic） | zag-ai | 已落地基础；H6 deadline/cancel 收口 |
...
| run_shell | ✅ permission + policy | deadline/cancel/error shape；OS sandbox remains C7 |

## 持久化

| 文件 | 内容 | 阶段 |
|------|------|------|
| `.zag/sessions/*.jsonl` | transcript | schema v1 已有；H/P0 补 explicit open、atomic preservation、writer conflict |
| `.zag/traces/*.jsonl` | audit events | H/P0-P1 补 truthful terminal、visible I/O、schema version |
