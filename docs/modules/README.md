...
| [cli-interaction.md](./cli-interaction.md) | Product CLI → M0 ✅; errno hotfix ✅ `bc737025` (`ci-hang-sigint-linux-errno-001`); [CI fuses](../plan/tasks/ci-hang-ci-fuses-001.md) docs **in-progress**; process-idle + Linux Gate still planned | `packages/zag-cli/src/{cli,sigint}.zig` + core cancel flag; CI fuses: [quality/README](../quality/README.md) | 保持 signal UX 在产品层；raw Linux errno 不得走 libc `posix.errno`；CI timeout ≠ product proof |
