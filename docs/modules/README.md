...
| [tui-minimal.md](./tui-minimal.md) | M2 / C9 **contract candidate** | binding only (`tui-minimal-001`); round-1 BLOCKED fixed; re-review pending; no product TUI | unique `packages/zag-tui`; dual-thread host; redactAlloc; `-Dtui` default false; impl BLOCKED until re-review PASS |
...
| [tui-minimal.md](./tui-minimal.md) | M2 / C9 contract candidate | Minimal host TUI binding (`tui-minimal-001`; re-review pending after blocker close; impl blocked) |
