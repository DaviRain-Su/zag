...
| [tui-minimal.md](./tui-minimal.md) | M2 / C9 **done** @ `f8f7f55` (contract PASS @ `c7a8f3a`) | `packages/zag-tui` + CLI `-Dtui` wire (`tui-minimal-001`); dual final reviews PASS; local macOS Gates; **no** maturity raise | unique `packages/zag-tui`; dual-thread host; redactAlloc; `-Dtui` default false |
...
| [tui-minimal.md](./tui-minimal.md) | M2 / C9 **done** @ `f8f7f55` (contract PASS @ `c7a8f3a`) | Minimal host TUI binding + `packages/zag-tui` implementation (`tui-minimal-001`; no maturity raise) |
