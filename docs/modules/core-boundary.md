...
core-observation-ownership-001   ✓ done — Trace/redaction/Observer moved to coding-agent; Core emits LoopEvent facts only
        │
        ▼
core-policy-ownership-001   impl committed (task branch; pending merged-main closeout) — concrete permissions/HITL/remember, workspace Guard/Root/realpath/symlink containment, and shell protect/off moved to coding-agent; Core retains required ports + deniedBody renderers + pure lexical `tool_args.checkToolPath`
        │
        ▼
