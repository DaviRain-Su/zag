...
| 代码（规划） | C5 按真实 use case 设计 optional port + 后端 `.zag/memory/`；H 不预留空 hook |
...
## Memory Core future port（C5 设计）

H/SDK minimum contract 不加入 `NullMemory` 或空 hook。C5 在 repo map/session contracts 稳定后，以真实 read/search/write/inject use case 定义端口。候选形状（非当前 public API）：
...
- H 阶段：不存在 Memory runtime/hook。
- C5：先写 use case/contract，再接 Memory Repo；关闭时与 H 出口零行为变化。
...
```text
H session/context correctness
  → C5.1 repo map
  → C5.2 session fork
  → C5.3 optional compaction upgrade
  → C5.4 Memory Repo MVP（default-off）
  → optional embed retrieval（never the only path）
```
