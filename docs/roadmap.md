...
M2 — selected daily UX
  skills-001 ✅ caafef5 → prompt-templates-001 ✅ 61326ae
        ...
        └──────────────┬──────────────┘
                       ▼
                 tui-minimal-001 (contract PASS @ c7a8f3a; impl candidate awaiting review)
```
...
| [tui-minimal-001](./plan/tasks/tui-minimal-001.md) | **contract PASS** @ `c7a8f3a` + **impl candidate** ([tui-minimal.md](./modules/tui-minimal.md)): host UI over public lifecycle/control/permission; awaiting independent review + coordinator Gates | only `packages/zag-tui`; dual-thread host; no maturity raise; no Linux/remote claim |
...
| C9 Product shell | minimal TUI **contract PASS** @ `c7a8f3a` + **impl candidate** ([tui-minimal-001](./plan/tasks/tui-minimal-001.md); awaiting independent review + coordinator Gates; no maturity raise) | `rpc-v1`, themes, extension UI host, ACP, dashboard/images/full configuration UX |
