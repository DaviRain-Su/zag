...
E1 begins with `skills-001` (binding module [skills.md](../../modules/skills.md); **done** at `caafef5`; Runtime Extensions remains L0), then a separately specified Prompt Template surface (`prompt-templates-001`; binding [prompt-templates.md](../../modules/prompt-templates.md); **done** at `61326ae`; coding-agent + thin CLI routing; maturity still L0). Theme data follows the host TUI; a runtime Custom Model catalog is a distinct validated configuration task, not executable extension code. Passive resources carry no loader execution privilege, but their content may still induce ordinary Tool/shell calls, which remain subject to permission, containment, shell policy, and redaction.
