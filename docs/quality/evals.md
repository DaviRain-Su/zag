...
2. explicit unwritable / invalid trace path is observable (`TraceIoFailed` / `InvalidPath`); provider not called on preflight failure. ✅
3. completed, max-turns, and cancelled each have one matching terminal event. ✅
4. session save failure → `ok=false`, `session_error` (not completed success). ✅
5. `Agent.deinit` does not invent a success terminal. ✅
6. `schema_version` present on `run_start`; duplicate `run_end` guarded. ✅
