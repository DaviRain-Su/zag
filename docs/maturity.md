...
| Trace / Observability | **L2** | h-trace-001: `schema_version`；facade 单 terminal；provider/session/max_turns/cancel 路径 truthful；`TraceIoFailed`/`InvalidPath` 可观测；deinit 不发明成功；redaction 仍 P1 | versioned schema ✅；exactly one truthful terminal ✅；visible persistence failure ✅；redact P1 | dashboard/correlation |
