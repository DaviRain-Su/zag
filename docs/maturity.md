...
| Session / Resume | **L2** | D-006 durable contract unchanged；create/resume distinct、atomic prior-byte preservation、one active writer、strict header、lexical session path；`a5ff2b7` adds Session-owned bounded control queues as process-only state, with pending slots deliberately absent from schema v1. `session-fork-001` ([session-fork](./modules/session-fork.md)): task-branch implementation landed and independent code review PASS; status remains **in-progress** until root dual-backend + merged-main Gate. Session row stays **L2** — **not** an L3 tree/journal/parent_id claim and **not** done. Not claimed: fsync/power-loss, symlink containment, hostile Writer-copy defense, L3 fork/tree/UI | explicit create/resume；atomic preservation；visible save errors；exclusive writer/conflict ✅；transient control isolation/retention ✅ | L3 fork/tree/journal as needed |
...
- repository-owned external stateful consumer test ✅ (`tests/sdk-consumer-fixture/` was **7/7** when the Gate closed; it is now **20/20** after D-011, `harness-events-001`, and `harness-steering-001`, without reopening or reclassifying the Gate)
