...
|-----------|:-------:|----------------------|---------|--------------|
| Loop / Turn | **L1+** | soft Tool errors、serial order、goldens、between-call cancel、facade 单 terminal（h-trace-001）已有；in-flight cancel/deadline 未闭合 | API/error/trace terminal 一致 ✅；cancel/deadline 有界；≥2 goldens | steer、parallel read-only |
...
| Trace / Observability | **L2** | h-trace-001: `schema_version`；facade 单 terminal；non-destructive preflight + atomic replace；per-reply latest-run；transactional writeObj；fail-closed 优先级；truthful stop_reason（含 OOM/invalid_toolset）；redaction 仍 P1 | versioned schema ✅；one truthful terminal ✅；visible/atomic persistence ✅；redact P1 | dashboard/correlation |
