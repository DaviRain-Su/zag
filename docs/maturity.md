...
| Zig source composition | **L2** | SDK-ready Gate remains closed at `ebdd7ab`：stateful Tool、high-level injection、ownership/lifetime/error/event/per-run cancel/session contracts、external consumer CI；coding-agent lifecycle closed at `aecf402`, Session control/Core `ControlInput` at `a5ff2b7`, idle-only `Session.fork` at `0a3087f`, and E1 Skills public options/activation at `caafef5` as enrichment without a new maturity row；current fixture **22/22** | supported import surface + contract docs + `tests/sdk-consumer-fixture/` pass merged-main Gate | published packages after second consumer |
...
| Runtime Extensions | L0 | D-010 + feature correspondence；trusted static Zig composition is SDK L2；E1 Skills **slice done** at `caafef5` (`skills-001` — [skills](./modules/skills.md); coding-agent discovery/catalog/`read_skill`/activation + §11 fixtures in `skills_tests.zig` + SDK **22/22**)；E1 Prompt Templates **docs contract in-progress** (`prompt-templates-001` — [prompt-templates](./modules/prompt-templates.md); not implemented)；E2 process / E3 WASM host / runtime bundle/UI do not exist；no separate E1 maturity Gate has raised the row；**row stays L0** | Prompt Templates implementation after independent contract PASS；separate E1 maturity Gate only if explicitly scheduled；E2 requires C7.1；E3 requires WIT/runtime/capability/package Gates；untrusted native requires C7.2 | local bundle + host-rendered stateful UI + separately gated WASM Provider/hook worlds |
...
- repository-owned external stateful consumer test ✅ (`tests/sdk-consumer-fixture/` was **7/7** when the Gate closed; it is now **22/22** after D-011, `harness-events-001`, `harness-steering-001`, `session-fork-001`, and `skills-001`, without reopening or reclassifying the Gate)
