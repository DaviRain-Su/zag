...
| Runtime Extensions | L0 | D-010 + feature correspondence；trusted static Zig composition is SDK L2；E1 Skills **slice done** at `caafef5` (`skills-001` — [skills](./modules/skills.md); coding-agent discovery/catalog/`read_skill`/activation + §11 fixtures in `skills_tests.zig` + SDK **22/22**) but Prompt Templates / E2 process / E3 WASM host / runtime bundle/UI do not exist；no separate E1 maturity Gate has raised the row；**row stays L0** | separate E1 Skills maturity Gate then Prompt Templates；E2 requires C7.1；E3 requires WIT/runtime/capability/package Gates；untrusted native requires C7.2 | local bundle + host-rendered stateful UI + separately gated WASM Provider/hook worlds |
