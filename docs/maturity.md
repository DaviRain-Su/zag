...
| Session / Resume | **L2** | D-006 durable contract unchanged；create/resume distinct、atomic prior-byte preservation、one active writer、strict header、lexical session path；`a5ff2b7` adds Session-owned bounded control queues as process-only state, with pending slots deliberately absent from schema v1. `session-fork-001` binding docs are in-progress ([session-fork](./modules/session-fork.md)); fork is **not implemented** and does **not** move this row. Not claimed: fsync/power-loss, symlink containment, hostile Writer-copy defense, L3 fork/tree | explicit create/resume；atomic preservation；visible save errors；exclusive writer/conflict ✅；transient control isolation/retention ✅ | fork/tree/journal as needed |
