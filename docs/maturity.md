...
| Provider / zag-ai | **L1+** | two wire styles、retry/usage/cost；**std+curl deadline/cancel + partial Tool discard**（h-provider-001）；redact 仍 P1 | enforced deadline ✅；in-flight cancel ✅；redaction；contract matrix | fallback/multi-key/third protocol on demand |
...
| Quality / Evals | **L1+** | goldens、provider fixtures、dual backend CI；tool-policy + session + symlink + trace + context + **provider deadline/cancel** fixtures 已进；redaction 等仍缺 | P0/P1 failure matrix进入 CI；不得削弱断言 | edit/cost/performance baselines |
...
7. **Deadline/cancel**：公开 timeout 真正执行（std+curl；默认 null 不强加）；provider/stream cancellation 有界；半截 Tool call 不执行（h-provider-001 ✅；tool/shell mid-flight 仍开）。
8. **Editing/runtime**：search_replace/grep/glob 可用；shell exit/timeout/truncation/deny 为稳定机器可读形状。
9. **Observability**：trace schema versioned ✅；permission/jail/shell/usage/stop reason 可复盘；显式 trace I/O failure 可见 ✅；secret redaction 仍 P1。
