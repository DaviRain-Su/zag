...
**总状态（文档日）：** Teaching = L1 齐；Production Floor = **未达 L2**；Capability = 未开始。
...
|--------|:----:|------|-------------------|---------|------|
| Loop / Turn | L1 | `src/agent/loop.zig` | 统一可机读 tool 错误；cancel 干净结束；max_turns/超时进 trace；≥2 golden | 并行只读、steer、Turn 生命周期 | Pi loop；Nanocodex Turn |
...
| Context / Compaction | L1 | `src/agent/context.zig`（截断 view） | 四层 prompt；超限 compaction（摘要+最近 N）落盘可解释 | repo map；智能选文件 | Pi；Aider；Hyper |
| Session / Resume | L1 | `session_store.zig` | schema 版本 + 迁移；transcript≠view 边界写死 | session 树 / fork / 旁支 | Pi；Nanocodex fork |
| Provider / zag-ai | L1 | `packages/zag-ai/` | 错误分类+重试成文；usage 进 session/trace；流式取消规格；≥1 家 contract | fallback / multi-key；多协议 | Hyper models；omp |
| Trace / Observability | L1 | `src/agent/trace.zig` | schema 版本化；能复盘 permission/jail/shell/usage | dashboard；费用透视 | Hyper dashboard |
...
| Quality / Evals | L0 | 少量单测 | H 起：golden + security eval 可 CI | edit eval；cost 基线 | Nanocodex contracts |
