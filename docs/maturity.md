...
| Session / Resume | **L2** | D-006: create/resume distinct; open_or_create SDK-only; atomic save + failpoint preserves prior bytes; save errors surface via Agent/CLI; one active writer via reusable `{path}.lock` advisory lock; strict header parser; lexical session path; P0 fixtures (create-existing, resume missing/invalid/unsupported, fault-save, busy, stale sidecar). Not claimed: fsync/power-loss or symlink containment | explicit create/resume; atomic preservation; visible save errors; exclusive writer/conflict | fork/tree/journal as needed |
