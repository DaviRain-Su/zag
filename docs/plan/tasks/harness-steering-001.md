...
- every repository-owned low-level `loop.Deps` composer
...
2. **Fixed bounded storage:** Session start preallocates four 4096-byte slots per kind before durable create/write.
   Enqueue copies bytes without allocation and returns `QueueFull`, `MessageTooLong`, `EmptyMessage`, or `InvalidUtf8`
   without mutation. No truncation or overwrite is allowed.
3. **Thread claim is narrow:** one foreign thread may enqueue while one reply consumes. Concurrent replies,
   clear/deinit during reply, Agent re-entry, and signal-handler enqueue remain unsupported.
4. **Explicit Core seam:** every `loop.Deps` provides `ControlInput`; low-level no-control composition selects
   `.none()` explicitly. The port exposes non-destructive `peek(kind)` plus infallible `commit(kind)`; Core owns no
   queue/mutex/capacity/persistence.
5. **Apply transaction:** append one user row to Transcript, commit that queue head, then emit source-backed
   `LoopEvent.control_applied`. Append OOM leaves the item queued. Sink failure after commit is visible and leaves the
   applied row in memory.
6. **One-at-a-time insertion:** at most one control item feeds one provider turn. Pre-turn steering, between-Tool
   steering, and would-complete steering/follow-up are the only v1 insertion points. Steering precedes follow-up at a
   would-complete boundary.
7. **No mid-flight preemption:** steering does not cancel Provider.chat or an entered Tool handler. Cancellation checked
   before a boundary wins when observed and consumes no control item.
8. **Protocol legality:** before mid-batch steering inserts a user row, every remaining accepted/not-started Tool gets
   one end-only result with the original id/name and `code=steered`; no synthetic `tool_start`. Real cancellation keeps
   `code=cancelled`.
...
12. **Observation:** Core extends the existing `LoopEvent` source union; coding-agent projects borrowed
    `LifecycleEvent.control_applied { kind, next_turn, text }`. Observer, Trace v1 twelve-kind schema, `headless-v1`, and
...
2. Pre-turn steering appends one user row before the next `turn_start`; one-at-a-time prevents a second control item
   from feeding the same provider turn.
3. Provider returns two Tools; steering before Tool 2 yields Tool 1 start/end, Tool 2 end-only `code=steered`, then user
   control, then the next provider turn. The projected view remains protocol legal.
4. Would-complete steering wins over follow-up; follow-up alone continues the same run. Both retain FIFO order.
5. Cancel observed before apply emits no control event and leaves queue state uncommitted.
6. Append OOM does not commit; source-sink failure after commit returns the existing visible sink/trace error mapping.
7. Last-turn pending control is not appended and produces `max_turns`; last-turn Tools finish normally.
...
9. Capacity, max bytes, empty, UTF-8, FIFO, pending counts, clear, and Session-start preallocation OOM are deterministic.
10. Session A queue cannot be consumed by Session B. A Session move while idle remains valid; active operations require
    stable address.
11. A barrier-based foreign thread enqueue during `reply` is observed at the next eligible boundary without allocation
    races or deadlock.
