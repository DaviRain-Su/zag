...
`origin/main`; this closeout did not execute or authorize that push. TUI docs
tips were local-only at closeout and are later ancestors of TARGET `f352b60`
(Class C lineage only; not Phase B live evidence; closeout did not push);
remote branch presence is not a Linux/remote Gate. Post-TUI default-path remote
dual-backend Gate is owned by
[post-tui-remote-dual-backend-gate-001](./tasks/post-tui-remote-dual-backend-gate-001.md)
(**in-progress**, rebind PASS @ `7f9cfa4`): TARGET
`f352b60d08e81c19d70ba46198fb06b71ddc85a1` (OLD_TARGET `b151307` abandoned);
**no** run id; **no** Gate green; **no** push; **no** Phase B grant; Phase B
needs fresh authz naming TARGET; **no** remote `-Dtui`; historical M0 tip/run
...
| [zag-live-002](./tasks/zag-live-002.md) | **ready** | D-014 Route A (reordered): live prompt surface in coding-agent; contract [live-policy-layer.md](../modules/live-policy-layer.md) dual PASS; implementation unblocked by zag-live-004 |
| [zag-live-003](./tasks/zag-live-003.md) | **draft** | Track L follow-on (provider bridge / prompt surface) |
| [live-runtime-spike-004](./tasks/live-runtime-spike-004.md) | **done** (2026-08-14) | Runtime comparison: full 10-probe matrix PASS under Chez / gxi / gsc-exe; review [pass](./reviews/live-runtime-spike-004-01.md); produced [D-015](../decisions/active/D-015-live-runtime-gerbil-gambit.md) (switch to Gerbil/Gambit) |
| [zag-live-004](./tasks/zag-live-004.md) | **done** | D-015 port: `packages/zag-live/` Chez→Gambit; impl [verify PASS](./reviews/zag-live-004-03-implementation.md) — 42/42 from clean build, 14 classes × both spawn forms |
...
|----|----------|--------|-------|
| [tui-minimal-001](./tasks/tui-minimal-001.md) | P1 | **done** @ `f8f7f55` | M2/C9 minimal host TUI; contract PASS @ `c7a8f3a` → impl final `f8f7f55` (dual final reviews PASS, zero blockers; PTY + exclusive gate21 workspace); local ff-only merge; task + merged-main local macOS default std **42/42 · 656/656**, curl **44/44 · 655/655**, TUI std **47/47 · 711/711**, TUI curl **49/49 · 710/710**; OpenAPI **287**; catalog **40**; docs **92/74** (55 files); binding [tui-minimal.md](../modules/tui-minimal.md); local reflog external/other push of `f8f7f55` to `origin/main` (not by this closeout); TUI docs tips local-only at closeout, later ancestors of TARGET `f352b60` (Class C lineage only; not Phase B live evidence); **no** maturity raise; remote Gate deferred to [post-tui-remote-dual-backend-gate-001](./tasks/post-tui-remote-dual-backend-gate-001.md) |
...
| [session-item-001](./tasks/session-item-001.md) | P1 | **implementation-complete** @ `31523b6` | M3.1 additive session-item semantics: reasoning/synthetic/prompt-index fields, token estimates, repair, and token-budget view truncation; independent closeout review record remains pending |
| [chat-state-prune-001](./tasks/chat-state-prune-001.md) | P1 | **implementation-complete** @ `31523b6` | M3.2 carrier-scoped duplicate tool-result dedup + prompt-index rewind API; view-only and fail-closed validation preserved |
| [compaction-llm-001](./tasks/compaction-llm-001.md) | P1 | **implementation-complete** @ `31523b6` | M3.3 optional LLM compaction summary with retry/fallback; heuristic behavior remains the floor |
