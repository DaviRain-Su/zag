...
| Core responsibility correction | **session ownership done** at `b137723` — [D-011](../decisions/active/D-011-thin-agent-core-boundary.md); Trace/policy/context ownership moves remain serialized |
| Next code task | `core-observation-ownership-001` **ready**; `harness-events-001` remains re-queued behind the migration |
...
completed foundation
  Phase H ──► SDK-ready
   ...
   core-session-ownership-001 (done) ✅
                  │
                  ▼
 core-observation-ownership-001 (ready)
...
`pi-alignment-001`, `cli-sigint-001`, the D-011 docs node `core-boundary-001`, seam step `core-seams-001`, and durable session move `core-session-ownership-001` are complete. Source review found that the earlier lifecycle design would add a third Core event channel while leaving product policy/state in the kernel. `core-observation-ownership-001` is now the only ready implementation node; all code nodes are serialized, and `harness-events-001` returns only after the thin-Core migration. Task priorities express safety impact; the dependency chain, not priority labels, fixes delivery order.
...
|----|----------|--------|-------|
| [core-observation-ownership-001](./tasks/core-observation-ownership-001.md) | P0 | ready | Trace/redaction/logging ownership → coding-agent |
