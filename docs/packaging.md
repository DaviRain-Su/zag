...
L5 产品面        zag-cli ✅           flags · resolve · one-shot / REPL
                 zag-tui / zag-acp   （C9；later package **only** `packages/zag-tui` per [tui-minimal](./modules/tui-minimal.md); contract PASS @ `c7a8f3a` — no package yet; `-Dtui` default false; CLI wires when true）
