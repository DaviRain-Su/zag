...
L5 产品面        zag-cli ✅           flags · resolve · one-shot / REPL
                 zag-tui / zag-acp   （C9；**only** `packages/zag-tui` per [tui-minimal](./modules/tui-minimal.md); **implemented/closed** @ `f8f7f55` (contract PASS @ `c7a8f3a`); `-Dtui` default true; omit with `-Dtui=false`; no maturity raise；zag-acp still planned）
...
E1 Skills Gate at `caafef5` passed std **609/609**, curl **608/608**, and SDK fixture **22/22** at that closeout.
Prompt Templates later enriched the external fixture to **23/23** at `61326ae` without reopening the SDK-ready Gate.
None of these enrichments changed a maturity row (Runtime Extensions remains L0). See [`modules/core-boundary.md`](./modules/core-boundary.md).
