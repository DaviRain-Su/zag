...
- **Phase H**：保持当前 package layout；既有 session/Tool/containment/trace/context/redact/provider/shell Gates 保持有效；final audit 新增 `h-edit-integrity-001` 与 `h-read-search-bounds-001`，integration blocked 到两者完成。
